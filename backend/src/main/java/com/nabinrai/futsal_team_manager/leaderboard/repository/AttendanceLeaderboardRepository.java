...
public interface AttendanceLeaderboardRepository
    ...
    @Query("""
            SELECT new com.nabinrai.futsal_team_manager.leaderboard.dto.response.AttendanceLeaderboardProjection(
                ...
                SUM(
                    CASE
                        WHEN mp.status = com.nabinrai.futsal_team_manager.match.enums.ParticipationStatus.ATTENDED
                        THEN 1
                        ELSE 0
                    END
                ),
                SUM(
                    CASE
                        WHEN mp.status = com.nabinrai.futsal_team_manager.match.enums.ParticipationStatus.NO_SHOW
                        THEN 1
                        ELSE 0
                    END
                )
